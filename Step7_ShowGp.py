...
if __name__ == "__main__":
    ...
    Label_MSLB_inside,   =     fig.text(0.5, 0.10, f'MSLB inside : {net_out_MSLB_inside[-1]}', wrap=True, backgroundcolor='#eefade'),
    Label_MSLB_outside,  =     fig.text(0.5, 0.05, f'MSLB outside : {net_out_MSLB_outside[-1]}', wrap=True, backgroundcolor='#eefade'),
    ...
    def update(frame):
        ...
        if is_updated:
            ...
            with open('GP_Result_team.csv', 'a') as f:
                f.write(f'{cnsenv.mem["KCNTOMS"]["Val"]},{net_out_NORMAL[-1]},{net_out_LOCA[-1]},{net_out_SGTR[-1]},{net_out_MSLB_inside[-1]},{net_out_MSLB_outside[-1]},{NetDiag},{out}\n')
