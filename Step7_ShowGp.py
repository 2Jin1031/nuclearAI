...
if __name__ == "__main__":
    ...
    def input_transform(data):
        ...
        pca = joblib.load('setting/pca_transformer.joblib')
        etc_data_arr = pca.transform(etc_data)
        
        etc_index = [f'extra value {i}' for i in range(10)]
        
        etc_data = pd.DataFrame(columns=etc_index) if do_pca else pd.DataFrame()
        ...
        # minmax 수행
        minmax_db = pd.read_csv('setting/minmax.csv')
    ...
    def update(frame):
        ...
        if is_updated:
            ...
            model = joblib.load('setting/model.joblib')
