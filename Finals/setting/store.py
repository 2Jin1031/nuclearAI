...
def store(selected_variable:list, do_pca:bool, case:int):
    ...
    # PCA 모델 저장
    dump(d.pca, f'./files_by_cases/pca_transformer_case{case}.joblib')

    # min, max 값은 추후 사용을 위해 저장해줍시다 ==========================================================
    with open(f'./files_by_cases/minmax_case{case}.csv', 'w') as f:
    ...
    dataset.to_csv(f'./files_by_cases/dataset_case{case}.csv')


# case1 (only selecting variables)
store(['ZINST22', 'UCTMT', 'WSTM1', 'WSTM2', 'WSTM3', 'ZINST102'], False, 1)
print("finish")
